...
bool City::determineNewPosition(int& r, int& c, int dir) const
{
    switch (dir)
    {
        case Game::UP:     if (r <= 1)      return false; else r--; break;
        case Game::DOWN:   if (r >= rows()) return false; else r++; break;
        case Game::LEFT:   if (c <= 1)      return false; else c--; break;
        case Game::RIGHT:  if (c >= cols()) return false; else c++; break;
...
void City::display() const
{
    ...
    char grid[globals::MAXROWS][globals::MAXCOLS];
    int r, c;
...
bool City::addFlatulan(int r, int c)
{
    ...
    if (m_nFlatulans == globals::MAXFLATULANS)
        return false;
